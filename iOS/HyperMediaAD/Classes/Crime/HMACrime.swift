...
/// MARK: - HMACrime
class HMACrime: NSManagedObject {
     ...
     * @return Array<HMACrime>
     */
    class func fetch(#location: CLLocation, radius: Double) -> Array<HMACrime> {
        var context = HMACoreDataManager.sharedInstance.managedObjectContext

        var fetchRequest = NSFetchRequest()
        let entity = NSEntityDescription.entityForName("HMACrime", inManagedObjectContext:context)
        ...
        let crimes = context.executeFetchRequest(fetchRequest, error: &error) as! Array<HMACrime>
        return crimes
    }
    ...
    class func save(#json: JSON, date: NSDate) {
        ...
        for crimeData in crimeDatas {
            HMACrime.insertCrime(json: crimeData, date: date)
        }
    ...
    class func insertCrime(#json: JSON, date: NSDate) {
        ...
        for child in children {
            HMACrime.insertCrime(json: child, date: date)
        }
    }
}
